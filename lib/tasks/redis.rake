...
# https://github.com/defunkt/resque/blob/master/lib/tasks/redis.rake
#
...
class RedisRunner
  ...
  def self.stop
    "#{bin_dir}/redis-cli -p 6379 SHUTDOWN"
  end
end
...
namespace :redis do
  ...
  task :start do
    require 'redis'
    begin
      @@redis ||= Redis.new; @@redis.dbsize
    rescue
      @retry_count ||= 3
      puts "Redis server not found, attempting to start... attempts remaining: #{@retry_count}"
      ...
      @retry_count -= 1
      t.join
      raise Exception.new "Couldn't start redis" if @retry_count == 0
      retry
    end
