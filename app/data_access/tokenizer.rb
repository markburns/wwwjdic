class Tokenizer
  STOP_WORDS = %w|a able about across after all almost also am among an and
  ...
  def tokens
    unless @query =~ /"/
      tokens = @query.split " "
      tokens.delete_if{|t| STOP_WORDS.include? t}
      return tokens
    end

    quoted_regex = /"([^\"]+)"/
    quoted = @query.scan quoted_regex

    quoted << @query.gsub(quoted_regex, "").split(" ")
    quoted.flatten.compact
  end
end
